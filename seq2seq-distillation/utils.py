...
class Seq2SeqDataset(AbstractSeq2SeqDataset):
    ...
    def collate_fn(self, batch) -> Dict[str, torch.Tensor]:
        ...
        # print("DATASET KWARGS", self.dataset_kwargs)
        batch_encoding: Dict[str, torch.Tensor] = self.tokenizer(
            [x["src_texts"] for x in batch],
            max_length=self.max_source_length,
            truncation=self.max_source_length > 0, 
            padding=True, 
            return_tensors="pt",
            # **self.dataset_kwargs,
        ).data
        target_batch_encoding = self.tokenizer(
            [x["tgt_texts"] for x in batch],
            max_length=self.max_target_length,
            truncation=self.max_target_length > 0, 
            padding=True, 
            return_tensors="pt",
            # **self.dataset_kwargs,
        ).data
        ...
        batch_encoding["labels"] = target_batch_encoding["input_ids"]
