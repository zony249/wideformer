...
from dataclasses import make_dataclass, replace, asdict
...
class SummarizationModule(BaseTransformer):
    ...
    def generate(self, batch: dict) -> dict:
        ...
        gen_time = (time.time() - t0) / batch["input_ids"].shape[0]
        preds: List[str] = generated_ids
        target: List[str] = batch["labels"]
        loss_tensors = self._step(batch)
        # base_metrics = dict(zip(self.loss_names, loss_tensors))
        # rouge: Dict = self.calc_generative_metrics(preds, target)
        # summ_len = np.mean(lmap(len, generated_ids))
        # base_metrics.update(gen_time=gen_time, gen_len=summ_len, preds=preds, target=target, **rouge)
        return loss_tensors, preds, target
...
def main(args, hf_training_args, model=None) -> SummarizationModule:
...
if __name__ == "__main__":
    ...
    hf_training_args = TrainingArguments(args.output_dir, 
                                         ...
                                         metric_for_best_model="bleu", save_steps=10000)
    

    # hf_training_args.__class__ = make_dataclass('TrainingArgumentsAug', 
    #                                             fields=[], #[(k, type(v)) for k, v in vars(args).items()], 
    #                                             bases=(TrainingArguments,))
    main(args, hf_training_args)
