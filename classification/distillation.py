...
from trainer import DistillTrainer, PrefixCollator
import torch
from torch import multiprocessing 


from make_student import init_student_from_teacher, init_student_from_random 
...
class ModelArguments:
    ...
    lora_adapter: Optional[str] = field(
        default=None, metadata={"help": 
                                """
                                Path to lora adapter. Will raise exception if lora adapter does not exist.
                                Accept three states: 
                                    None: Training is done without LoRA 
                                    random_init: Randomly initialized LoRA 
                                    path: loads specific LoRA from path.
                                """}
    )
    ...
    num_student_layers: Optional[int] = field(
        default=3, 
        metadata={"help": "Number of student layers"}
    )
    student_init_strategy: Literal["weight_copy", "random_init"] = "weight_copy"
...
class CustomTrainingArguments(TrainingArguments): 
    ...
    is_regression: Optional[bool] = field(
        default=False, 
        metadata={"help": "Whether the task is regression or not. This is automatically set when data_args.task_name == stsb"} 
    )
    ...
    kl_alpha: Optional[float] = field(
        default = 1.0, 
        metadata = {"help": "kl loss balancing coefficient"}
    )
    ce_alpha: Optional[float] = field(
        default = 1.0, 
        metadata = {"help": "cross-entropy balancing coefficient"}
    )
...
def main():
    ...
    if model_args.lora_adapter is not None: 
        if model_args.lora_adapter == "random_init": 
            ...
            model = PeftModel.from_pretrained(model, model_args.lora_adapter)
    ...
    if label_to_id is not None:
        ...
        model.config.id2label = {id: label for label, id in model.config.label2id.items()} #config.label2id.items()}
